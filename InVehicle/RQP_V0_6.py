# Purpose: Make road quality measurement
#
# Save to RPi as RQP_Prod.py for autolaunch on boot
#
...
#                Now calculating and storing difference between raw and filtered z accel measurements
#                Periodic measurments as well as high g trigger added
# 2017 09 21 AJL OLED banner will now alternate between project name, output file name and time of day
# 2017 10 16 AJL External LED will light when in Bump data collection mode
#                Output file opened when in logging mode for first time.
#
# To launch this code on boot of the RPi
# add an appropriate line to the end of /etc/rc.local
# example: (sleep 10; python3 /home/pi/Documents/python/code/RQP_V0_5.py)
#
# BOM
# ==============================================================
# Raspberry Pi Zero W
...
def NowSQL():
    ...
    # 2017 02 04 AJL Created function
    # 2017 02 10 AJL Corrected error where single digit dates were not having hyphens added
    # 2017 09 16 AJL substituted underscores for colons in time string because of JSON parsing error
...
def Date2FileName():
    ...
    # 2017 09 16 AJL Created function
...
MaxDIF = 500 # unscaled reading
LogNow = 0
nPoints = 0
Msg = "Road Quality Project"
LastMsg = time.time()
MsgInterval = 5
MsgCntr = 0
GPIO.output(bmpPin, GPIO.LOW)

# OLED screen positions - calculate a 1 pixel high horizontal bar and offset to get width
xLabel = 1
xData = 28
LowerLeft = 0
LowerRight = 128
...
while True:
    ...
    # read switch and set mode - default is STANDBY
    ...
    # form the message for display on the top row of the OLED
    if  time.time() - LastMsg > MsgInterval:
            LastMsg = time.time()
            MsgCntr = MsgCntr + 1
            if MsgCntr == 1:
                Msg = FileName  + ".csv"
    ...
    with canvas(device) as draw:
        ...
        print("GPIO 23 ", GPIO.input(bmpPin))
        
        # draw.text (xPixels, yPixels) font is 6 x 11
        draw.text((xLabel, 0), Msg, fill="white")
